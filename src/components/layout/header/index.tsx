...
import { Bell, ClipboardList, Gift, Headset, Heart, House, Info, PartyPopper, ShoppingCart, User } from "lucide-react";
    ...
    <>
      <div className="flex items-center flex-grow dark:bg-zinc-800 dark:text-zinc-50">
        ...
        <div className="flex items-center gap-3">
          ...
          <ul className="flex gap-3 px-4 border-x border-zinc-200 py-3.5">
          ...
          <Button className="bg-transparent text-zinc-700 font-tajawal hover:bg-white pe-16 hover:text-zinc-700 shadow-none">
            {t("language")}
          </Button>
        </div>
      </div>
      <div className="dark:bg-softPink-200">
        <ul className="flex items-center justify-center gap-3 px-4 border-x bg-maroon-700 text-zinc-50 border-zinc-200 py-3.5">
          {links.map((item, index) => (
            <li key={index} className="mx-2">
              <Link href={item.href} className="flex items-center gap-2">{item.icon}{item.label}</Link>
            </li>
          ))}
        </ul>
      </div>
