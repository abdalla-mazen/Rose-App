...
import { useOccasionsSchema } from "@/lib/schemas/occasions.schema";
...
export default function UpdateOccasionsForm({ data }: { data: Occasion }) {
  ...
  //  Form
  const schema = useOccasionsSchema();
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 w-3/5  ">
        ...
        <Button
          ...
          variant={"primary"}
        >
          {isPending ? <LoaderCircle className="animate-spin" /> : t("update-Occasion")}
        </Button>
      </form>
    </Form>
  );
