...
//               <Button
//   asChild
//   className="w-full h-11 bg-maroon-600 hover:bg-maroon-700 text-white font-semibold rounded-xl shadow-md shadow-rose-100 transition-all"
// >
//   <Link
//     href="/addresses"
    ...
    <div className="flex items-center gap-1 mt-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg
          ...
          className={`w-3 h-3 ${
            star <= Math.round(avg)
              ? "text-amber-400 fill-amber-400 dark:text-softPink-400 dark:fill-softPink-400"
              : "text-gray-200 fill-gray-200 dark:text-softPink-900 dark:fill-softPink-900"
          }`}
          viewBox="0 0 20 20"
        >
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
      ...
      <span className="text-xs text-gray-400 dark:text-softPink-600 ml-0.5">({count})</span>
    </div>
  );
}
...
export default function CartClient({ dataCart }: CartClientProps) {
  ...
  return (
    <div className="min-h-screen bg-rose-50/50 dark:bg-zinc-800">
      {/* Header */}
      <div className="bg-white dark:bg-zinc-800 border-b border-rose-100 dark:border-softPink-600 sticky top-0 z-10 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <ShoppingBag className="text-rose-500 dark:text-softPink-400 w-6 h-6" />
            <h1 className="capitalize text-2xl font-bold text-gray-900 dark:text-softPink-50">
              {t("cart")}
            </h1>
            {cartItems.length > 0 && (
              <Badge className="bg-rose-100 text-rose-600 hover:bg-rose-100 dark:bg-softPink-700 dark:text-softPink-300 dark:hover:bg-softPink-900 border-0 px-3 font-bold">
                {cartItems.length} <span className="p-2">{t("cart-products")}</span>
              </Badge>
            )}
          </div>
          {cartItems.length > 0 && (
            <Button
              ...
              disabled={isPending}
              onClick={() => clear()}
              className="capitalize bg-maroon-50 text-maroon-600 hover:bg-maroon-100 dark:bg-softPink-300 dark:text-zinc-700 dark:hover:bg-softPink-800 dark:border-softPink-800 gap-2 w-fit"
            >
              <BrushCleaning className="w-3.5 h-3.5" />
              {t("clear-cart")} {isPending && <Loader2 className="animate-spin" />}
            </Button>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div className="flex-1 space-y-3">
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item) => {
              ...
              return (
                <Card
                  key={item._id}
                  className="border-rose-100 dark:border-softPink-400 dark:bg-zinc-800 shadow-sm hover:shadow-md transition-all duration-200"
                >
                  <CardContent className="p-4">
                    <div className="flex items-center gap-4">
                      {/* Image */}
                      <div className="relative rounded-xl overflow-hidden flex-shrink-0 bg-rose-50 dark:bg-softPink-500 border border-rose-100 dark:border-softPink-800">
                        <Image
                          src={item.product.imgCover}
                          alt={item.product.title}
                          className="w-full h-full object-cover"
                        ...
                        />
                        {hasDiscount && (
                          <Badge className="absolute top-1 left-1 bg-rose-500 dark:bg-softPink-600 text-white text-[9px] px-1 py-0 h-3.5 rounded-sm border-0">
                            {t("sale")}
                          </Badge>
                        )}
                      </div>
                      {/* Title + Rating + Price */}
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-800 dark:text-softPink-100 text-sm leading-snug line-clamp-2">
                          {item.product.title}
                        </p>
                        <StarRating avg={item.product.rateAvg} count={item.product.rateCount} />
                        <div className="flex items-baseline gap-2 mt-1.5">
                          <span className="font-bold text-gray-900 dark:text-softPink-50">
                            {item.product.priceAfterDiscount.toLocaleString()} EGP
                          </span>
                          {hasDiscount && (
                            <span className="text-xs text-gray-400 dark:text-softPink-600 line-through">
                              {item.product.price.toLocaleString()}
                            </span>
                          )}
                        </div>
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <button
                          ...
                          className="flex items-center gap-2 px-3 py-2 text-sm capitalize bg-red-600 text-white hover:bg-red-700 dark:bg-softPink-600 dark:hover:bg-softPink-800 rounded-md"
                        >
                          <Trash2 className="w-4 h-4" /> {t("remove")}{" "}
                          {isRemovePending && removeVariables?.productId === item.product._id && (
                            <Loader2 className="animate-spin" />
                        ...
                        {/* Qty Control */}
                        <div className="flex items-center border border-rose-200 dark:border-softPink-800 rounded-lg overflow-hidden bg-white dark:bg-softPink-500">
                          <button
                            onClick={() => updateQty(item._id, item.product._id, -1)}
                            className="w-8 h-8 flex items-center justify-center text-rose-400 dark:text-white hover:bg-rose-50 dark:hover:bg-softPink-900 transition-colors text-lg font-medium"
                          >
                            −
                          </button>
                          <span className="w-8 h-8 flex items-center justify-center text-sm font-semibold text-gray-800 dark:text-softPink-100 border-x border-rose-100 dark:border-softPink-800">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => updateQty(item._id, item.product._id, 1)}
                        ...
                        <span className="text-xs font-semibold text-rose-500 dark:text-softPink-400">
                          {lineTotal.toLocaleString()} EGP
                        </span>
                      </div>
                    </div>
          ...
          {cartItems.length > 0 && (
            <Button
              asChild
              variant="outline"
              className="md:w-1/3 w-full gap-2 bg-maroon-600 hover:bg-maroon-700 hover:text-white text-white dark:bg-softPink-700 dark:hover:bg-softPink-600 rounded-xl mt-1"
            >
              <Link href="/" className="capitalize flex items-center gap-2">
                <MoveLeft className="w-4 h-4" />
                {t("continue-shopping")}
        ...
        {cartItems.length > 0 && (
          <div className="w-full lg:w-72 flex-shrink-0">
            <Card className="border-rose-100 dark:border-softPink-500 dark:bg-zinc-800 shadow-sm sticky top-24">
              <CardContent className="p-5 space-y-4">
                <h2 className="capitalize font-bold text-gray-900 dark:text-softPink-50 text-base">
                  {t("order-summary")}
                </h2>
                <div className="space-y-2.5 text-sm">
                  <div className="capitalize flex justify-between text-gray-500 dark:text-softPink-400">
                    <span>{t("original-price")}</span>
                    <span className="line-through text-gray-400 dark:text-softPink-600">
                      {originalTotal.toLocaleString()} EGP
                    </span>
                  </div>
                  {savings > 0 && (
                ...
                <Separator className="bg-rose-100 dark:bg-softPink-900" />
                <div className="flex justify-between font-bold text-gray-900 dark:text-softPink-50">
                  <span>{t("total")}</span>
                  <span className="text-rose-600 dark:text-softPink-400 text-lg">
                    {subtotal.toLocaleString()} EGP
                  </span>
                </div>
                {savings > 0 && (
                  <div className="capitalize bg-green-50 dark:bg-green-950 border border-green-100 dark:border-green-900 rounded-lg px-3 py-2 text-xs text-green-700 dark:text-green-400 font-medium text-center">
                    {t("you-saved")} {savings.toLocaleString()} EGP!
                  </div>
                )}
                <Button
                  asChild
                  className="w-full h-11 bg-maroon-600 hover:bg-maroon-700 dark:bg-softPink-700 dark:hover:bg-softPink-600 text-white font-semibold rounded-xl shadow-md shadow-rose-100 dark:shadow-softPink-950 transition-all"
                >
                  <Link
                    href="/addresses"
                    className="flex w-full h-full items-center justify-center gap-2"
                ...
                <p className="text-center text-xs text-gray-400 dark:text-softPink-600">
                  {t("secure-checkout")}
                </p>
              </CardContent>
            </Card>
